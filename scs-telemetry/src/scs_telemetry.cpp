...
bool check_min_version(
    unsigned const int min_ets2,
    unsigned const int min_ats
    ) {
    return telemetryPtr->scs_values.game == ETS2 && telemetryPtr->scs_values.version_minor >= min_ets2
            || telemetryPtr->scs_values.game == ATS && telemetryPtr->scs_values.version_minor >= min_ats;
}

bool check_max_version(
    unsigned const int max_ets2,
    unsigned const int max_ats
    ) {
    return telemetryPtr->scs_values.game == ETS2 && telemetryPtr->scs_values.version_minor <= max_ets2
            || telemetryPtr->scs_values.game == ATS && telemetryPtr->scs_values.version_minor <= max_ats;
}

void log_scs_named_value(const scs_named_value_t *current) {
    switch (current->value.type) {
        ...
        case SCS_VALUE_TYPE_fplacement:
            logger->debug(
                Log::format(
                    " %s fplacement = (%f,%f,%f) h:%f p:%f r:%f",
                    current->name,
        ...
        case SCS_VALUE_TYPE_dplacement:
            logger->debug(
                Log::format(
                    " %s dplacement = (%f,%f,%f) h:%f p:%f r:%f",
                    current->name,
...
// some parts may not visible due to limited history
 ...
 */
scs_timestamp_t lastTimestamp = scs_timestamp_t(-1);
scs_timestamp_t lastSimulatedTimestamp = scs_timestamp_t(-1);
scs_timestamp_t lastRenderTimestamp = scs_timestamp_t(-1);
...
void set_trailer_values_zero(unsigned int trailerId = 0) {
    // (&telemetryPtr->trailer.trailer[trailerId])->~scsTrailer_t(); // slow
    // new(&telemetryPtr->trailer.trailer[trailerId]) scsTrailer_t(); // unsure about memory leaks
    memset(&telemetryPtr->trailer.trailer[trailerId], 0, sizeof(scsTrailer_t)); // plain old memset
}
...
SCSAPI_VOID telemetry_frame_start(
    const scs_event_t UNUSED(event),
    ...
    if (telemetryPtr != nullptr) {
        telemetryPtr->time = static_cast<uint64_t>(timestamp);
        telemetryPtr->simulatedTime = static_cast<uint64_t>(simulatedTimestamp);
        telemetryPtr->renderTime = static_cast<uint64_t>(renderTimestamp);
        ...
        // refuel = true, but engine is active? -> refuel is finished and payed
        ...
        // update last value every few ticks
        if (fuel_ticker > 10) {
            fuel_ticker = 0;
...
SCSAPI_VOID telemetry_gameplay(
    ...
    if (strcmp(info->id, SCS_TELEMETRY_GAMEPLAY_EVENT_job_cancelled) == 0) {
        ...
        logger->warn(
            Log::format(
                "Something went wrong with this gameplay event %s",
                info->id));
    }
    ...
    for (auto current = info->attributes; current->name; ++current) {
        if (!handleGpe(current, type)) {
            logger->info(
                Log::format(
                    "attribute not handled id: %i attribute: %s",
                    type,
                    current->name));
        }
    }
...
// called if the game fires the event configuration.
// Used to handle all the configuration values
    ...
    if (strcmp(info->id, SCS_TELEMETRY_CONFIG_substances) == 0) {
        ...
        if (check_max_version(13, 0)) {
            if (strcmp(info->id, SCS_TELEMETRY_CONFIG_trailer) == 0) {
                ...
                logger->warn(
                    Log::format(
                        "Something went wrong with this configuration %s",
                        info->id));
            }
            ...
            if (strstr(info->id, SCS_TELEMETRY_CONFIG_trailer)) {
                ...
                if (trailer_id > 9 || trailer_id < 0) {
                    logger->warn(
                        Log::format(
                            "Something went wrong while parsing trailer id",
                            info->id));
                } else {
...
SCSAPI_VOID telemetry_store_float(
    const scs_string_t UNUSED(name),
...
SCSAPI_VOID telemetry_store_bool(
    ...
    if (!value ||value->value_bool.value == 0) {
...
SCSAPI_VOID telemetry_store_fvector(
    const scs_string_t UNUSED(name),
    const scs_u32_t UNUSED(index),
    const scs_value_t* const value,
    scs_context_t context
...
SCSAPI_VOID telemetry_store_dplacement(
    const scs_string_t UNUSED(name),
    const scs_u32_t UNUSED(index),
    const scs_value_t* const value,
    scs_context_t context
...
SCSAPI_VOID telemetry_store_fplacement(
    const scs_string_t UNUSED(name),
    const scs_u32_t UNUSED(index),
    const scs_value_t* const value,
    scs_context_t context
...
SCSAPI_VOID telemetry_frame_end(
    ...
    server->broadcast(
        reinterpret_cast<char*>(telemetryPtr),
        sizeof(*telemetryPtr));
}
...
SCSAPI_RESULT scs_telemetry_init(
    ...
    for (auto i = scs_u32_t(0); i < WHEEL_SIZE; i++) {
        ...
        REGISTER_CHANNEL_INDEX(TRUCK_CHANNEL_wheel_susp_deflection, float, telemetryPtr->truck_f.truckWheelSuspensionDeflection[i], i);
    ...
    if (check_max_version(13, 0)) {
        ...
        REGISTER_CHANNEL(TRAILER_CHANNEL_local_linear_velocity, fvector, telemetryPtr->trailer.trailer[0].com_fv.linearVelocityX);
        REGISTER_CHANNEL(TRAILER_CHANNEL_local_angular_velocity, fvector, telemetryPtr->trailer.trailer[0].com_fv.angularVelocityX);
        REGISTER_CHANNEL(TRAILER_CHANNEL_local_linear_acceleration, fvector, telemetryPtr->trailer.trailer[0].com_fv.linearAccelerationX);
        REGISTER_CHANNEL(TRAILER_CHANNEL_local_angular_acceleration, fvector, telemetryPtr->trailer.trailer[0].com_fv.angularAccelerationX);
        REGISTER_CHANNEL(TRAILER_CHANNEL_wear_chassis, float, telemetryPtr->trailer.trailer[0].com_f.wearChassis);

        for (auto i = scs_u32_t(0); i < WHEEL_SIZE; ++i) {
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_susp_deflection, float, telemetryPtr->trailer.trailer[0].com_f.wheelSuspensionDeflection[i], i);
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_on_ground, bool, telemetryPtr->trailer.trailer[0].com_b.wheelOnGround[i], i);
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_substance, u32, telemetryPtr->trailer.trailer[0].com_ui.wheelSubstance[i], i);
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_velocity, float, telemetryPtr->trailer.trailer[0].com_f.wheelVelocity[i], i);
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_steering, float, telemetryPtr->trailer.trailer[0].com_f.wheelSteering[i], i);
            REGISTER_CHANNEL_INDEX(TRAILER_CHANNEL_wheel_rotation, float, telemetryPtr->trailer.trailer[0].com_f.wheelRotation[i], i);
        }
    }
    ...
    if (check_min_version(14, 1)) {
        ...
        for (auto i = 0; i < 10; ++i) {
            ...
            REGISTER_CHANNEL_TRAILER(i, velocity.angular, fvector, telemetryPtr->trailer.trailer[i].com_fv.angularVelocityX);
            REGISTER_CHANNEL_TRAILER(i, acceleration.linear, fvector, telemetryPtr->trailer.trailer[i].com_fv.linearAccelerationX);
            REGISTER_CHANNEL_TRAILER(i, acceleration.angular, fvector, telemetryPtr->trailer.trailer[i].com_fv.angularAccelerationX);
            ...
            for (auto j = scs_u32_t(0); j < WHEEL_SIZE; j++) {
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.suspension.deflection, float, telemetryPtr->trailer.trailer[i].com_f.wheelSuspensionDeflection[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.on_ground, bool, telemetryPtr->trailer.trailer[i].com_b.wheelOnGround[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.substance, u32, telemetryPtr->trailer.trailer[i].com_ui.wheelSubstance[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.angular_velocity, float, telemetryPtr->trailer.trailer[i].com_f.wheelVelocity[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.steering, float, telemetryPtr->trailer.trailer[i].com_f.wheelSteering[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.rotation, float, telemetryPtr->trailer.trailer[i].com_f.wheelRotation[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.lift, float, telemetryPtr->trailer.trailer[0].com_f.wheelLift[j], j);
                REGISTER_CHANNEL_TRAILER_INDEX(i, wheel.lift.offset, float, telemetryPtr->trailer.trailer[0].com_f.wheelLiftOffset[j], j);
            }
        }
    }
...
SCSAPI_VOID scs_telemetry_shutdown() {
    ...
    server->broadcast(reinterpret_cast<char*>(telemetryPtr), sizeof(*telemetryPtr));
